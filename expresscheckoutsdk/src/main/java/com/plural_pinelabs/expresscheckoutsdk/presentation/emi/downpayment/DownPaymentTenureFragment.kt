package com.plural_pinelabs.expresscheckoutsdk.presentation.emi.downpayment
...
class DownPaymentTenureFragment  : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_down_payment_tenure, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val slider = view.findViewById<Slider>(R.id.down_payment_slider)
        val tvAmount = view.findViewById<TextView>(R.id.tv_down_payment_amount)
        val tvMin = view.findViewById<TextView>(R.id.tv_down_payment_min)
        val tvMax = view.findViewById<TextView>(R.id.tv_down_payment_max)

// 🔥 Dynamic values from backend
        val allowedValues = listOf(0, 5000, 10000, 20000, 30000, 39750)

// Configure slider
        slider.valueFrom = 0f
        slider.valueTo = (allowedValues.size - 1).toFloat()
        slider.stepSize = 1f
        slider.value = 0f

// Set min/max labels
        tvMin.text = "₹${allowedValues.first()}"
        tvMax.text = "₹${allowedValues.last()}"

// Initial value
        tvAmount.text = "₹${allowedValues[0]}"

// Listen for changes
        slider.addOnChangeListener { _, value, _ ->
            val index = value.toInt()
            val selected = allowedValues[index]
            tvAmount.text = "₹$selected"
        }
